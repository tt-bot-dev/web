...
import type { Config } from "@tt-bot-dev/types";

export default function isBotOwner(config: Config, user: string): boolean {
    if (typeof config.ownerID === "string") {
        return config.ownerID === user;
    } else {
        return config.ownerID.includes(user);
    }
}
