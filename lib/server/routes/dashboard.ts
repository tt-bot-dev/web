...
export default function (app: Polka, csrfProtection: typeof import("csurf"), config: Config, bot: TTBotClient): void {
    ...
    app.get("/dashboard/:id/extensions/:extension", checkAuth(), csrfProtection(), async (rq, rs) => {
        const guilds = getGuilds(bot, config, rq, rs);
        if (!guilds) return;
        if (!guilds.find(g => g.isOnServer && g.id === rq.params.id)) return rs.sendStatus(403);
        else {
            ...
            if (rq.params.extension === "new") {
                ...
                await render(rs, "dashboard-extensions-update", makeTemplatingData(rq, bot, config, {
                    erisGuild,
                    extension: {
                        id: "new"
                    },
            ...
            const extension = await bot.db.getGuildExtension(rq.params.extension);
            if (!extension || extension?.guildID !== erisGuild.id) {
                rs.status(404);
                await render(rs, "404", makeTemplatingData(rq, bot, config, {
                    pageTitle: "404"
                }));
                return;
            }
    ...
    app.get("/dashboard/:id/extensions", checkAuth(), csrfProtection(), async (rq, rs) => {
        const guilds = getGuilds(bot, config, rq, rs);
        if (!guilds) return;
        if (!guilds.find(g => g.isOnServer && g.id === rq.params.id)) return rs.sendStatus(403);
        else {
            ...
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            const erisGuild = bot.guilds.get(rq.params.id)!;
            const extensions = await bot.db.getGuildExtensions(rq.params.id);

            await render(rs, "dashboard-extensions", makeTemplatingData(rq, bot, config, {
                erisGuild,
                extensions,
                pageTitle: `Extensions for ${erisGuild.name}`,
                csrfToken: rq.csrfToken()
            }));
        }
    });
